...
// Thiết lập thông tin chữ và hiệu ứng màu
let hue = 0; // Biến hue để thay đổi màu sắc
const text = [
  "Chúc mừng năm mới!",
  "Chúc các cậu mãi xinh đẹp, mạnh khỏe, mọi điều tốt đẹp.",
  "Mong rằng năm mới các cậu sẽ lì xì cho.",
  "Tớ xin cảm ơn!"
];
const fontSize = 40;
const lineHeight = 60; // Khoảng cách giữa các dòng

function drawText() {
  ctx.clearRect(0, 0, canvas.width, canvas.height); // Xóa canvas mỗi lần vẽ lại chữ

  ctx.font = `${fontSize}px 'Arial'`; // Thử dùng Arial nếu Russo One không tải được
  ctx.textAlign = 'center';
  ctx.fillStyle = `hsl(${hue}, 100%, 60%)`; // Màu sắc thay đổi theo hue

  // Vẽ các dòng chữ căn giữa
  let y = canvas.height / 2 - ((text.length - 1) * lineHeight) / 2; // Căn chỉnh chữ theo chiều dọc
  for (let i = 0; i < text.length; i++) {
    ctx.fillText(text[i], canvas.width / 2, y); // Vẽ từng dòng chữ
    y += lineHeight; // Tăng khoảng cách giữa các dòng
  }

  // Thay đổi giá trị hue để màu sắc thay đổi liên tục
  hue += 1;
  if (hue > 360) hue = 0;
